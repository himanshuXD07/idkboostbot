...
if os.name == "nt":
    ctypes.windll.kernel32.SetConsoleTitleW(f"Boost Bot v1 | Invincible Tools | .gg/tweet")
else:
    pass
...
async def on_ready():
    ...
    fade_text = fade.purplepink(BoostBot)
    print(fade_text + '\n' + Account + '\n' + menu + '\n')
...
async def help(ctx: discord.ApplicationContext):
    ...
    embed = discord.Embed(title=f"**BOT COMMANDS**", color=0x2F3136) 
...
`/givetokens` - Allows you to give tokens to a user""")
    embed.set_thumbnail(url="https://media.discordapp.net/attachments/1198569216241061938/1198577342470180915/Hometown.png?ex=65c8a3a2&is=65b62ea2&hm=d48dd3b111876f8072aec27fa62fefe779f89dda7aeb1281a285b4473f21c091&=&format=webp&quality=lossless")
...
async def activity(ctx, activity=discord.Option(str, "activity", required=True)):
    ...
    await bot.change_presence(activity=discord.Streaming(type=discord.ActivityType.streaming, name=activity, url="https://www.twitch.tv/"))
    from colorama import Fore
    TimeNow = datetime.now().strftime(f"{Fore.LIGHTBLACK_EX}[{Fore.WHITE}%H{Fore.LIGHTBLACK_EX}:{Fore.WHITE}%M{Fore.LIGHTBLACK_EX}:{Fore.WHITE}%S{Fore.LIGHTBLACK_EX}]{Fore.RESET}")
...
async def ping(ctx):
    ...
    await ctx.respond(embed = discord.Embed(title = " ", description = f"{round(bot.latency * 1000)} ms**", color = 0x2c2f33))
...
async def restock(ctx, code: discord.Option(str, "Paste.ee link", required = True),type: discord.Option(int, "Type of tokens you are restocking, 3 months or 1 month", required=True)):
    ...
    embed = discord.Embed(title=f"**TOKENS RESTOCKED**",
...
async def addowner(ctx, member: discord.Option(discord.Member, "Member who has add to be added as an owner.", required = True)):
    ...
    embed = discord.Embed(title=f"<:retocks:1192766858659631175> | **ADDED OWNER**",
...
async def boost(ctx, invite: discord.Option(str, "Invite link to the server you want to boost.", required = True), amount: discord.Option(int, "Number of times you want to boost the sever.", required = True), months: discord.Option(int, "Number of months you want to boost the server for 1 or 3.", required = True),nick: discord.Option(str, "Nickname you want to set for the boosting account.", required = False) = config['server_nick'],bio : discord.Option(str, "Bio you want to set for the boosting account.", required = False) = config['set_bio']):
    if ctx.author.id not in config["ownerID"] and ctx.author.id not in config['adminID']:
        return await ctx.respond(embed = discord.Embed(title = "**Missing Permission**", description = "<a:redot:1192766322589839380> **You must be an owner or an administrator to use this command <a:redot:1192766322589839380>", color = 0xf46868))
    if months != 1 and months != 3:
        return await ctx.respond(embed = discord.Embed(title = "**Invalid Input**", description = "<a:redot:1192766322589839380> **Months can either be 3 (months) or 1 (month)** <a:redot:1192766322589839380>", color = 0xf46868))
    if amount % 2 != 0:
        return await ctx.respond(embed = discord.Embed(title = "**Invalid Input**", description = "<a:redot:1192766322589839380> **Amount needs to be even** <a:redot:1192766322589839380>", color = 0xf46868))
    if months == 1:
    ...
    if checkEmpty(filename):
        return await ctx.respond(embed = discord.Embed(title = "**Stock Error**", description = "<a:redot:1192766322589839380> **There is currently no stock in the files. Please use `/restock` to add nitro tokens in the stock files** <a:redot:1192766322589839380>", color = 0xf46868))
    if len(open(filename, "r").readlines()) < amount / 2:
        return await ctx.respond(embed = discord.Embed(title = "**Stock Error**", description = "<a:redot:1192766322589839380> **There is currently not enough stock in the files. Please use `/restock` to add nitro tokens in the stock files** <a:redot:1192766322589839380>", color = 0xf46868))
    
    invite = getinviteCode(invite)
    
    if validateInvite(invite) == False:
        return await ctx.respond(embed = discord.Embed(title = "**Invite Error**", description = "<a:redot:1192766322589839380> **The invite submitted is invalid. Please sumbit a valid invite link** <a:redot:1192766322589839380>", color = 0xf46868))
