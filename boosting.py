...
def sprint(message, type:bool):
    if type == True:
        print(f"{timestamp()} {Style.BRIGHT}{Fore.LIGHTGREEN_EX}{message}{Fore.RESET}{Style.RESET_ALL}")
    if type == False:
        print(f"{timestamp()} {Style.BRIGHT}{Fore.LIGHTRED_EX}{message}{Fore.RESET}{Style.RESET_ALL}")
    ...
    try:
        for i in range(10):
            ...
            if response.status_code == 429:
                ...
                r = response.json()
                solution = get_captcha_key(rqdata = r['captcha_rqdata'], site_key = r['captcha_sitekey'], websiteURL =f'https://discord.com/api/v9/invites/{invite}', useragent = useragent)
...
def put_boost(session, headers, guild_id, boost_id):
    try:
        ...
        if boosted.status_code == 201:
            ...
            #sprint(f"[{thread}] {Fore.LIGHTRED_EX}BOOST ON COOLDOWN : {Fore.LIGHTBLACK_EX}{token}", False)
            return False
    except Exception as e:
...
def change_bio(session, headers, server_id, bio):
    try:
        jsonPayload = {"bio": bio}
        r = session.patch(f"https://discord.com/api/v9/users/@me/profile", headers=headers, json=jsonPayload)
        if r.status_code == 200:
            return True
        else:
            return False
        ...
        #sprint(f"[{thread}] {str(e).capitalize()} | Function: Change_Guild_Name, Retrying", False)
        change_bio(session, headers, server_id, bio)
...
def boost_server(invite:str , months:int, token:str, thread:int, nick: str,bio: str):
    ...
    try:
        ...
        if "401: Unauthorized" in boost_data.text:
            sprint(f"[{thread}] {Fore.LIGHTRED_EX}INVALID: {Fore.LIGHTBLACK_EX}{token}", False)
        ...
        if "You need to verify your account in order to perform this action." in boost_data.text:
            sprint(f"[{thread}] {Fore.LIGHTRED_EX}LOCKED: {Fore.LIGHTBLACK_EX}{token}", False)
            variables.failed_tokens.append(token)
            remove(token, filename)
        ...
        if boost_data.status_code == 200:
            if len(boost_data.json()) != 0:
                ...
                if join_outcome:
                    sprint(f"[{thread}] {Fore.LIGHTYELLOW_EX}JOINED: {Fore.LIGHTBLACK_EX}{token}", True)
                    for boost in boost_data.json():
                        boost_id = boost["id"]
                        boosted = put_boost(session, headers, guild_id, boost_id)
                        if boosted:
                            sprint(f"[{thread}] {Fore.LIGHTMAGENTA_EX}BOOSTED: {Fore.LIGHTBLACK_EX}{token}", True)
                            variables.boosts_done += 1
                            if token not in variables.success_tokens:
                                variables.success_tokens.append(token)
                        else:
                            sprint(f"[{thread}] {Fore.LIGHTRED_EX}ERROR BOOSTING: {Fore.LIGHTBLACK_EX}{token}", False)
                    ...
                    if config["change_server_nick"]:
                        ...
                        if changed:
                            sprint(f"[{thread}] {Fore.LIGHTYELLOW_EX}RENAMED {Fore.RESET}({nick}): {Fore.LIGHTBLACK_EX}{token}", True)
                        else:
                            sprint(f"[{thread}] {Fore.LIGHTRED_EX}ERROR RENAMING: {Fore.LIGHTBLACK_EX}{token}", False)

                    if config["change_bio"]:
                        changedd = change_bio(session, headers, guild_id, bio)
                        if changedd:
                            sprint(f"[{thread}] {Fore.LIGHTYELLOW_EX}CHANGED BIO {Fore.RESET}({bio}): {Fore.LIGHTBLACK_EX}{token}", True)
                        else:
                            sprint(f"[{thread}] {Fore.LIGHTRED_EX}ERROR CHANGING BIO: {Fore.LIGHTBLACK_EX}{token}", False)
                else:
                    sprint(f"[{thread}] {Fore.LIGHTRED_EX}ERROR JOINING: {Fore.LIGHTBLACK_EX}{token}", False)
                    variables.failed_tokens.append(token)
            else:
                remove(token, filename)
                sprint(f"[{thread}] {Fore.LIGHTRED_EX}NO NITRO: {Fore.LIGHTBLACK_EX}{token}", False)
                variables.failed_tokens.append(token)
...
def thread_boost(invite, amount, months, nick, bio):
    ...
    if validateInvite(invite) == False:
        sprint(f" {Fore.LIGHTRED_EX}[ - ] {Fore.RESET}The invite received is invalid.", False)
        return False
    ...
    while variables.boosts_done != amount:
        ...
        if len(tokens) == 0 or len(tokens) < numTokens:
            sprint(f" {Fore.LIGHTRED_EX}[ - ] {Fore.RESET}Not enough {months} month(s) tokens' stock left to complete request", False)
            return False
            ...
            for i in range(numTokens):
                sprint(f" {Fore.LIGHTGREEN_EX}[ {i+1} ] {Fore.RESET}Thread Started", True)
                threads[i].start()
            print()
